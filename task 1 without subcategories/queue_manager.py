...
def worker():
    ...
    # create client session that will be used for all requests in this stream
    ...
    try:
        ...
        # Close the session and event loop correctly
        loop.run_until_complete(session.close())
        loop.close()

def db_writer():
    # continue processing until the completion flag is not set, or the queue is not empty
...
def start_workers():
    ...
    # Launching data to write data in the database
...
def stop_workers(threads):
    shutdown_event.set()
    # waiting until the lines are completely processed
